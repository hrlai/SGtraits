...
load_database <- function(doi = NULL, version = NULL, path = "data/SGtraits", update = FALSE){
    ...
    if(! file.exists(file_nm)){
        # Downloading file
        download_database(url, file_nm, path = path)
    }
...
load_json <- function(path, update){
    ...
    #### FIX ME generalise JSON filename #####
    file_json <- file.path(path, "SGtraits.json")

    # Does the .json exist?
    if(! file.exists(file_json) | update == TRUE){
        # Retrieve the .json
        res <- jsonlite::read_json("https://zenodo.org/api/records/16249456/versions",
                                   simplifyVector = T)
        # Save it
...
download_database <- function(url, filename, path) {
    ...
    message("Downloading database to '", path,"'")
    ...
    if (res == 0) { #Warning and timeout hygiene
        ...
        stop('Could not download database!' )
    }
}
